...
export default function SearchDisplay({ initialData, initialRateLimit }: SearchDisplayProps) {
  ...
  const [results, setResults] = useState<GithubUser[]>(() => initialData.items);
  ...
  useEffect(() => {
    if (initialRateLimit.limit > 0 || initialRateLimit.remaining > 0) {
      dispatch(
        setRateLimit({
          isAuth: initialRateLimit.isAuth,
          limit: initialRateLimit.limit,
          remaining: initialRateLimit.remaining,
        }),
      );
    }
  }, [dispatch, initialRateLimit.isAuth, initialRateLimit.limit, initialRateLimit.remaining]);
