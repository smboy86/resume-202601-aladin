...
export default function SearchRepositoriesPage() {
  ...
  const buildQuery = () => {
    ...
    if (rangeMode === 'range') {
      ...
      if (!start || !end) return '';
      return `${typeFilter} ${identityFilter}repos:${start}..${end}${keyword ? ` ${keyword}` : ''}`;
    }
    const value = singleValue.trim();
    if (!value) return '';
    const operator = rangeMode === 'gte' ? '>=' : '<=';
    return `${typeFilter} ${identityFilter}repos:${operator}${value}${keyword ? ` ${keyword}` : ''}`;
  };
    ...
    <div className='min-h-screen bg-zinc-50 px-6 py-12 text-zinc-900 sm:px-10 dark:bg-zinc-950 dark:text-zinc-50'>
      <main className='mx-auto flex w-full max-w-4xl flex-col gap-8'>
        <header className='space-y-2'>
          ...
          <h1 className='text-3xl font-semibold tracking-tight'>사용자가 소유한 리포지토리 수로 검색</h1>
        ...
        <Paper className='rounded-2xl border border-zinc-200 p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900'>
          ...
          <div className='sm:flex-start mt-4 flex flex-col gap-3'>
            ...
            <Button
              ...
              disabled={
                loading ||
                (rangeMode === 'range' ? !rangeStart.trim() || !rangeEnd.trim() : !singleValue.trim())
              }
